"""Run M20: per-bar detection after a reset, with caps set where they can bind.

Research only. Reads M16's canonical 4h CSVs and writes evidence under ``var/research/m20/``.
...
Usage::
    ...
    uv run python scripts/m20_study.py [--workers 2] [--only XRPUSDT] [--policies G,H12]
"""
...
def job(asset_index: int, policy_index: int) -> dict[str, Any]:
    ...
    policy = POLICIES_M20[policy_index]
