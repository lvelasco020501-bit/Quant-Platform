"""Run M21: an account stop that closes open exposure when the global cap breaks.

Research only. Reads M16's canonical 4h CSVs and writes evidence under ``var/research/m21/``.
Touches no paper state, no production risk and no running session.

Each job is one market under one policy, over exactly M18's four scenarios, so H, I and J are
measured against C, E and G on identical ground. What is added is the budget: every run also
records the loss from the **original** high-water mark, whether a budget was spent, and whether
a market that ended halted for good had actually spent one.
...
Usage::
    ...
    uv run python scripts/m21_study.py [--workers 2] [--only XRPUSDT] [--policies S12]
"""
...
from quantplatform.research.m19 import local_overshoot
from quantplatform.research.m21 import POLICIES_M21
...
def _opened_after_stop(result: ExperimentResult, engine: RecoveryLatchRiskEngine | None) -> int:
    """Return how many positions were opened *after* the account stopped. Must be zero.

    Strictly after, and the distinction is not pedantry. A position can open *on* the stop
    bar, because the order that opened it was approved before the stop existed and filled at
    that bar's open; the account stop then closes it on the next bar, which is the earliest
    anything can. Counting that as an entry after the stop would report a violation where the
    invariant actually held.
    """
    stopped = engine.stopped_at if engine is not None else None
    if stopped is None:
        return 0
    return sum(1 for trade in result.trades if trade.opened_at > stopped)


def _realised_at_stop(
    result: ExperimentResult, engine: RecoveryLatchRiskEngine | None
) -> str | None:
    """Return what the position closed by the account stop realised, if one was closed."""
    stopped = engine.stopped_at if engine is not None else None
    if stopped is None:
        return None
    closed = [t for t in result.trades if t.closed_at >= stopped]
    return str(closed[0].realized_pnl) if closed else None
...
def run_once(
    ...
    *,
    ...
    return {
        ...
        "global_drawdown": (
            result.performance.max_drawdown if result.performance is not None else None
        ),
        "budget_exhausted": engine.budget_exhausted if engine is not None else None,
        "bars_seen": engine.bars_seen if engine is not None else 0,
        "stopped_at": engine.stopped_at if engine is not None else None,
        "forced_exits": [
            {
                "symbol": a.symbol,
                "at": a.at,
            ...
            for a in (engine.actions_taken if engine is not None else [])
        ],
        "trades_after_stop": _opened_after_stop(result, engine),
        "realized_at_stop": _realised_at_stop(result, engine),
        "per_bar_detection": bool(engine is not None and engine.bars_seen > 0),
        "local_overshoot_per_halt": [
            str(
                (Decimal(str(e["reference_before"])) - Decimal(str(e["equity_at_halt"])))
                / Decimal(str(e["reference_before"]))
            )
            for e in episodes
            if e.get("reference_before") and e.get("equity_at_halt") is not None
        ],
        "resets": sum(1 for e in episodes if e.get("reference_after") is not None),
        "ended_permanently": bool(episodes) and episodes[-1].get("ended") is None,
        "permanent_without_cause": (
            bool(episodes)
            and episodes[-1].get("ended") is None
            and engine is not None
            and not engine.budget_exhausted
        ),
...
def job(asset_index: int, policy_index: int) -> dict[str, Any]:
    ...
    policy = POLICIES_M21[policy_index]
